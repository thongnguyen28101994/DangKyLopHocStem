...
export default function ListParticipantPaid() {
  ...
  const handleFindSchoolID = async () => {
    ...
    const response = await CommonApi.getOfficialParticipantPaid(SchoolID.SchoolID,SchoolID.CLASS_ID);
    if(response.StatusCode===200)
    {
      SetParticipant(response.Result)
    }
  };
  const handleFindSchoolID1 = async (SchoolID,ClassID) => {
    
    const response = await CommonApi.getOfficialParticipantPaid(SchoolID,ClassID);
    if(response.StatusCode===200)
    {
      SetParticipant(response.Result)
    }
  ...
  return (
    <>
      ...
      <PersonRegisterModal
        FGetParticipant={handleFindSchoolID1}
